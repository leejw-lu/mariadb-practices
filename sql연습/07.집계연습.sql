-- 1) 집계쿼리: select 절에 통계함수 (count, max, min, sum, avg, variabce, stddev ...)
...
from salaries 
group by emp_no having avg_salary >=60000 
...
-- 
select emp_no, avg(salary), sum(salary) from salaries group by emp_no having emp_no;
