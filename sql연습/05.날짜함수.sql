...
select now(), sleep(2), now() from dual;		-- now는 select query가 실행되는 타임
select now(), sleep(2), sysdate() from dual;	-- sysdate는 함수가 실행된 타임
...
-- 예제) 근무개월
-- 		포맷팅: YYMM , YYYYMM
select first_name, hire_date, period_diff(date_format(curdate(), '%y%m'), date_format(hire_date, '%y%m')) as Month from employees;

-- date_add(=adddate), date_sub(=subdate)
-- 날짜 date 타입의 컬럼이나 값에 type(year, month, day)의 표현식으로 더하거나 뺄 수 있다.
