...
select a.first_name, b.title from employees a, titles b where a.emp_no = b.emp_no	-- join 조건 (n-1)
and b.to_date ='9999-01-01'; -- row 선택 조건

-- 예제2) 현재, 근무하고 있는 직원의 이름과 직책을 모두 출력하되, 여성 Engineer만 출력하세요.
...
-- 2) join ~ using
-- 	 natural join 문제점 -> 의도치 않은 컬럼이 join으로 걸릴 수도
...
group by a.title order by avg(b.salary) desc;

-- 실습문제1
-- 현재, 직원별 근무 부서를 출력해 보세요.
-- 사번, 지원이름(first_name), 부서명 순으로 출력하세요.
select a.emp_no, a.first_name, b.dept_name from employees a, departments b, dept_emp c
...
select a.title as 직책, avg(b.salary) as 평균연봉, count(*) as 직원수 from titles a join salaries b on a.emp_no=b.emp_no
where a.to_date='9999-01-01' and b.to_date='9999-01-01'
group by a.title having 직원수>=100;
...
group by a.title having count(*)>=100;

-- 실습문제4
-- 현재, 부서별로 직책이 Engineer인 직원들에 대해서만 평균 연봉을 구하세요.
-- projection: 부서이름 평균급여
select a.dept_name as 부서이름, avg(d.salary) as 평균급여
from departments a join dept_emp b on a.dept_no=b.dept_no join titles c on b.emp_no=c.emp_no join salaries d on c.emp_no=d.emp_no
where c.title='Engineer' and b.to_date='9999-01-01' and c.to_date='9999-01-01' and d.to_date='9999-01-01' 
group by a.dept_name order by 평균급여 desc;
