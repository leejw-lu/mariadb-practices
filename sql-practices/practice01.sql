...
-- 문제4.
-- 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) >>현재는 무조건 to_date==9999~~
...
-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외 => 현재 매니저만 출력하라는 말. 9999-01-01)
select count(*) from dept_manager where to_date='9999-01-01';

-- 문제7.
-- 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요. >> length
...
select title, from_date, to_date from titles where emp_no=13250 order by to_date;
